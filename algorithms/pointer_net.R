...
dist_mtrx <- as.matrix(dist(cities)) # distance matrix 
cities_tnsr <- torch_tensor(cities) # tensor version of task
glimpse_tnsr(cities_tnsr) # check task as tensor
...
attention <- nn_module(
  initialize = function(hidden_size, use_tanh=FALSE, C=10){
    ...
    # Bahdanau components
    self$W_query = nn_linear(hidden_size, hidden_size) # linear transformation for query
    self$W_ref   = nn_conv1d(hidden_size, hidden_size, 1, 1) # simple 1 dim convolution layer
    
    self$V = torch_zeros(hidden_size, dtype = torch_float()) |> nn_parameter()
...
tst_query <- torch_rand(1, 1, 128) # prepare for test
tst_ref <- torch_rand(1, 16, 128) # prepare for test
tst_idx <- torch_tensor(c(2,5))$to(torch_long()) # prepare for test
...
pointer_net <- nn_module(
  initialize = function(embedding_size, hidden_size, seq_len, tanh_exploration, use_tanh){
    ...
    self$embedder  = nn_linear(2, embedding_size, bias = F)
    self$encoder   = nn_lstm(embedding_size, hidden_size, batch_first=T)
    self$decoder   = nn_lstm(embedding_size, hidden_size, batch_first=T)
    self$pointer   = attention(hidden_size, use_tanh=use_tanh, C=tanh_exploration)
    self$glimpse   = attention(hidden_size, use_tanh=F)
    
    self$decoder_start_input = nn_parameter(torch_zeros(seq_len, embedding_size))
...
pointer_net(30, 128, 16, 10, TRUE)(torch_rand(16, 2)) # test pointer_net
...
#      - mdl:    trained model 
trainer <- function(model, train_data, max_grad_norm=2, n_epochs, beta){
  ...
  critic_exp_mvg_avg = torch_zeros(1)
...
new_task <- generate_task(seed = 2022)  
